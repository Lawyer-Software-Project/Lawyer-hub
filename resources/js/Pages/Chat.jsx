    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="w-1/4 bg-white border-r border-gray-300">
        ...
        <div className="overflow-y-auto h-screen p-3 mb-9 pb-20">
          {/* Contact Items */}
          {['Alice','Martin'].map((name, index) => (
            <div key={index} className="flex items-center mb-4 cursor-pointer hover:bg-gray-100 p-2 rounded-md">
              <div className="w-12 h-12 bg-gray-300 rounded-full mr-3">
                <img src={`https://placehold.co/200x/${Math.random().toString(16).slice(2, 8)}/ffffff.svg?text=ʕ•́ᴥ•̀ʔ&font=Lato`} alt="User Avatar" className="w-12 h-12 rounded-full" />
              </div>
              <div className="flex-1">
                <h2 className="text-lg font-semibold">{name}</h2>
                <p className="text-gray-600">Sample message...</p>
              </div>
            </div>
          ))}
        </div>
